...
client.on("message", (message) => {
  ...
  for(var i = 1; i < blacklist.length; i++) {
    if(new RegExp(blacklist[i], "i").test(content)) {
      ...
      var warns = userinfo[message.guild.id]["members"][message.author.id]["warns"];const { Client } = require('discord.js');
const Discord = require("discord.js");
const userinfo = require("./data/userinfo.json")
const client = new Client({ disableEveryone: true});
const fs = require('fs');
...
client.on("message", (message) => {
  ...
  for(var i = 1; i < blacklist.length; i++) {
    if(new RegExp(blacklist[i], "i").test(content)) {
      ...
      let channel = message.guild.channels.find('name', 'mod_logs');
      if (!channel) return;
      if(!userinfo[message.guild.id]["members"][message.author.id]["punishments"]) {
        userinfo[message.guild.id]["members"][message.author.id]["punishments"] = new Map();
      }
      var warns = userinfo[message.guild.id]["members"][message.author.id]["punishments"];
      var warnID = genRandom(5);
      while(true) {
        var valid = true;
        for(targGuild in userinfo) {
          for(targMemb in userinfo[targGuild]["members"]) {
            for(targWarn in userinfo[targGuild]["members"][targMemb]["punishments"]) {
              if(warnID === targWarn) {
                warnID = genRandom(5);
                valid = false;
              }

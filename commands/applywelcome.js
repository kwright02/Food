...
module.exports = {
    run: async (client, msg, connection, args) => {
      sendSurvey(client, client.config.surveys.applywelcome, msg, "Welcomer Application");
    },
    meta: {
        name: 'applywelcome',
        description: 'Apply for welcomer rank',
        usage: ''
    }
}
    ...
    setTimeout(async function() {
      ...
      await collector.on('collect', collectedMessage => {
        ...
        if (counter === Object.keys(questSet).length) {
          ...
          .setColor(0x42f471)
          .setAuthor(`Welcomer Application | ${msg.author.tag}`, client.user.avatarURL)
          var counter2 = 1;
